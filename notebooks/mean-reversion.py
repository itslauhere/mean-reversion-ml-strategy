...
This project implements a mean reversion backtesting engine with machine learning enhancements.

🧠 Strategy:
- Uses z-score to detect mean-reverting signals
- Adds Random Forest ML classifier to predict next-day returns
- Filters trades based on both statistical signals AND ML predictions

💾 Input:
- Historical price data in CSV format (must contain 'Date' and 'Close' or 'Adj Close')

📤 Output:
- Trade log CSVs (BUY/SELL signals, returns)
- Summary performance table (Sharpe Ratio, P&L, Drawdown, CAGR)
- Plots comparing strategy vs market
- ML performance classification report

✅ How to Use:
1. Place your historical CSV files inside a folder (e.g. `data/`)
2. Run the script:
       python mean_reversion.py
3. View outputs:
       - Trade logs saved as {asset_name}_trades_log.csv
       - Strategy summary in `strategy_summary_table.csv`
       - Bar chart of Sharpe, CAGR, etc.
...
if __name__ == "__main__":
    try:
        optimize_strategies("data")  # <- Adjust to your folder with CSVs
    except Exception as e:
